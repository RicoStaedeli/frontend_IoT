...
////////////
//// Create Chart for AirQuality
////////////
async function createAirGraph(baseConfigChart) {
    var data_air = await loadData("https://poop-tracker-48b06530794b.herokuapp.com/air_qualities")

    days = data_air.map(item => item.timestamp);
    smoke = data_air.map(item => item.smoke);
    lpg = data_air.map(item => item.lpg);
    co = data_air.map(item => item.co);

    let config_data_air = JSON.parse(JSON.stringify(baseConfigChart));

    //Set x axis 
    config_data_air.data.labels = days;

    //set existing line
    config_data_air.data.datasets[0].label = "CO";
    config_data_air.data.datasets[0].data = co;
    config_data_air.data.datasets[0].type = "line";

    //Add three graphs
    //set chart config poops
    config_data_air.data.datasets.push({
        label: "Smoke",
        type: "line",
        fill: false,
        backgroundColor: "#ed64a6",
